...
bool Loader::LoadOBJ(const char* path, std::vector<float>& vertexData, std::vector<unsigned int>& vertexAttributes) {
		...
		for(size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
			...
			for(size_t v = 0; v < fv; v++) {
				...
				if(!attrib.colors.empty()) {
					vertexData.push_back(attrib.colors[3 * idx.normal_index + 0]);
					vertexData.push_back(attrib.colors[3 * idx.normal_index + 1]);
					vertexData.push_back(attrib.colors[3 * idx.normal_index + 2]);
				}
